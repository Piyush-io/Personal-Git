...
def object_read(repo, sha):
    ...
    path = repo.repo_file("objects", sha[0:2], sha[2:])  # Corrected repo_file call
...
def cat_file(repo, obj, fmt=None):
    ...
    if obj is None:
        print(f"Object {obj} not found in repository.")
        return
...
def cat_file(repo, obj, fmt=None):
    obj = object_read(repo, object_find(repo, obj, fmt=fmt))
    if obj is None:
        print(f"Required Object not found in repository.")
        return
    sys.stdout.buffer.write(obj.serialize())
...
def log_graphviz(repo, sha, seen):
    ...
    if commit is None:
        print(f"Commit object with SHA {sha} not found in repository.")
        return

    short_hash = sha[0:8]
    message = commit.kvlm[None].decode("utf8").strip() if None in commit.kvlm else ""
    ...
    print(f'  c_{short_hash} [label="{short_hash}: {message}"]')
    ...
    for p in parents:
        p = p.decode("ascii")
        print(f'  c_{short_hash} -> c_{p[0:8]};')
...
def tag_create(repo, name, ref, create_tag_object=False):
    ...
    if create_tag_object:
        ...
        tag.kvlm[b'tagger'] = b'pg <pg@example.com>'
        # …and a tag message!
        tag.kvlm[None] = b"A tag generated by pg, which won't let you customize the message!"
...
def ref_create(repo, ref_name, sha):
    with open(repo.repo_file(repo, "refs/" + ref_name), 'w') as fp:
        fp.write(sha + "\n")
...
def object_resolve(repo, name):
    ...
    if hashRE.match(name):
        ...
        name = name.lower()
        prefix = name[0:2]
        path = repo.repo_dir(repo, "objects", prefix, mkdir=False)
        if path:
            rem = name[2:]
...
def object_find(repo, name, fmt=None, follow=True):
      ...
      while True:
          obj = object_read(repo, sha)
            # because we are reading the whole object there might be low performance
...
def index_read(repo):
    index_file = repo.repo_file(repo, "index")
    ...
    assert version == 2, "pg only supports index file version 2"
...
def gitignore_read(repo):
    ...
    # Read the global gitignore file in .git/info/exclude
...
def branch_get_active(repo):
    with open(repo.repo_file(repo, "HEAD"), "r") as f:
        head = f.read()
...
def index_write(repo, index):
    with open(repo.repo_file(repo, "index"), "wb") as f:
...
def cmd_commit(args):
    ...
    if active_branch: # If we're on a branch, we update refs/heads/BRANCH
        with open(repo.repo_file(repo, os.path.join("refs/heads", active_branch)), "w") as fd:
            fd.write(commit + "\n")
    else: # Otherwise, we update HEAD itself.
        with open(repo.repo_file(repo, "HEAD"), "w") as fd:
            fd.write("\n")